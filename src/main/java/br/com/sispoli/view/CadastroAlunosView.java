...
public class CadastroAlunosView extends JFrame {
    ...
    private JTextField txtLogradouro, txtBairro, txtCidade, txtCepObs;
    ...
    private static final Date EMPTY_DATE = Date.from(LocalDate.of(1900, 1, 1).atStartOfDay(ZoneId.systemDefault()).toInstant());
    ...
    private GridBagConstraints gbc(GridBagConstraints base, int x, int y, int w) {
        base.gridx = x; base.gridy = y; base.gridwidth = w; base.gridheight = 1;
        base.weightx = (w > 1) ? 1.0 : 0.1; base.weighty = 0; base.anchor = GridBagConstraints.WEST;
    ...
    private JPanel criarPanelDadosPessoais() {
        ...
        txtNome = new JTextField(30); p.add(txtNome, gbc(gbc, 1, 0, 3));
        
        p.add(new JLabel("CPF:"), gbc(gbc, 0, 1, 1));
        txtCPF = createMaskField("###.###.###-##"); txtCPF.setColumns(14); p.add(txtCPF, gbc(gbc, 1, 1, 1));
        
        p.add(new JLabel("RG:"), gbc(gbc, 2, 1, 1));
        txtRG = new JTextField(12); p.add(txtRG, gbc(gbc, 3, 1, 1));
        
        p.add(new JLabel("Nascimento:"), gbc(gbc, 0, 2, 1));
        spnDataNasc = createDateSpinner(); p.add(spnDataNasc, gbc(gbc, 1, 2, 1));
        
        p.add(new JLabel("Sexo:"), gbc(gbc, 2, 2, 1));
        cmbSexo = new JComboBox<>(new String[]{"", "Masculino", "Feminino", "Outro"}); p.add(cmbSexo, gbc(gbc, 3, 2, 1));
        
        p.add(new JLabel("Email:"), gbc(gbc, 0, 3, 1));
        txtEmail = new JTextField(25); p.add(txtEmail, gbc(gbc, 1, 3, 3));
        
        p.add(new JLabel("WhatsApp:"), gbc(gbc, 0, 4, 1));
        txtWhatsApp = createMaskField("(##) #####-####"); txtWhatsApp.setColumns(14); p.add(txtWhatsApp, gbc(gbc, 1, 4, 1));
    ...
    private JPanel criarPanelEndereco() {
        ...
        // ✅ CEP como JComboBox editável
        p.add(new JLabel("CEP:"), gbc(gbc, 0, 0, 1));
        cmbCep = new JComboBox<>();
        cmbCep.setEditable(true);
        cmbCep.addItem("🔍 Selecione ou digite para filtrar...");
        cmbCep.setToolTipText("Selecione na lista ou digite CEP/logradouro");
        p.add(cmbCep, gbc(gbc, 1, 0, 2));
        
        // Listener para auto-preenchimento
        cmbCep.addActionListener(e -> {
            String selecionado = cmbCep.getSelectedItem() != null ? cmbCep.getSelectedItem().toString() : "";
            if (selecionado.contains(" - ")) {
                String idCep = selecionado.split(" - ")[0];
                preencherCamposEndereco(idCep);
            }
        });
        
        p.add(new JLabel("Logradouro:"), gbc(gbc, 3, 0, 1));
        txtLogradouro = new JTextField(25); p.add(txtLogradouro, gbc(gbc, 4, 0, 2));
        
        p.add(new JLabel("Número:"), gbc(gbc, 0, 1, 1));
        txtNumero = new JTextField(8); p.add(txtNumero, gbc(gbc, 1, 1, 1));
        
        p.add(new JLabel("Complemento:"), gbc(gbc, 2, 1, 1));
        txtComplemento = new JTextField(15); p.add(txtComplemento, gbc(gbc, 3, 1, 1));
        
        p.add(new JLabel("Bairro:"), gbc(gbc, 4, 1, 1));
        txtBairro = new JTextField(15); p.add(txtBairro, gbc(gbc, 5, 1, 1));
        
        p.add(new JLabel("Cidade:"), gbc(gbc, 0, 2, 1));
        txtCidade = new JTextField(20); p.add(txtCidade, gbc(gbc, 1, 2, 2));
        
        p.add(new JLabel("Estado:"), gbc(gbc, 3, 2, 1));
        cmbEstado = new JComboBox<>(new String[]{"AC","AL","AP","AM","BA","CE","DF","ES","GO","MA","MT","MS","MG","PA","PB","PR","PE","PI","RJ","RN","RS","RO","RR","SC","SP","SE","TO"});
        p.add(cmbEstado, gbc(gbc, 4, 2, 1));
        
        p.add(new JLabel("Obs. Endereço:"), gbc(gbc, 5, 2, 1));
        txtCepObs = new JTextField(15); p.add(txtCepObs, gbc(gbc, 6, 2, 1));
    ...
    private JPanel criarPanelMatricula() {
        ...
        spnDataMatricula = createDateSpinner(); p.add(spnDataMatricula, gbc(gbc, 1, 0, 1));
        ...
        chkIsento = new JCheckBox(); p.add(chkIsento, gbc(gbc, 5, 0, 1));
    ...
    private JPanel criarPanelEmergencia() {
        ...
        txtNomeEmergencia = new JTextField(25); p.add(txtNomeEmergencia, gbc(gbc, 1, 0, 2));
        
        p.add(new JLabel("Telefone:"), gbc(gbc, 3, 0, 1));
        txtTelEmergencia = createMaskField("(##) #####-####"); txtTelEmergencia.setColumns(14); p.add(txtTelEmergencia, gbc(gbc, 4, 0, 1));
        
        p.add(new JLabel("Parentesco:"), gbc(gbc, 5, 0, 1));
        cmbParentesco = new JComboBox<>(new String[]{"", "Pai", "Mãe", "Avó", "Avô", "Tio(a)", "Outro"}); 
    ...
    private JPanel criarPanelSaude() {
        ...
        txtRestricao = new JTextArea(2, 25); txtRestricao.setLineWrap(true); txtRestricao.setWrapStyleWord(true); txtRestricao.setEnabled(false); 
        p.add(new JScrollPane(txtRestricao), gbc(gbc, 2, 0, 3));
        
        p.add(new JLabel("Medicamentos Contínuos:"), gbc(gbc, 5, 0, 1));
        txtMedicamentos = new JTextArea(2, 20); txtMedicamentos.setLineWrap(true); txtMedicamentos.setWrapStyleWord(true); 
        p.add(new JScrollPane(txtMedicamentos), gbc(gbc, 6, 0, 2));
        
        p.add(new JLabel("Alergias:"), gbc(gbc, 0, 1, 8));
        txtAlergias = new JTextArea(2, 80); txtAlergias.setLineWrap(true); txtAlergias.setWrapStyleWord(true); 
    ...
    private JPanel criarPanelAutorizacoes() {
        ...
        p.add(chkImg); p.add(chkDivulg); p.add(chkTermos);
        p.add(new JLabel("Data Aceite:"));
        spnDataAceite = createDateSpinner(); spnDataAceite.setEnabled(false); p.add(spnDataAceite);
    ...
    private JPanel criarPanelCancelamento() {
        ...
        spnDataCancelamento = createDateSpinner(); spnDataCancelamento.setEnabled(false); p.add(spnDataCancelamento);
        
        p.add(new JLabel("Motivo:"));
        txtMotivoCancelamento = new JTextField(30); txtMotivoCancelamento.setEnabled(false); p.add(txtMotivoCancelamento);
    ...
    private JPanel criarPanelObsEBotoes() {
        ...
        txtObs = new JTextArea(4, 60); txtObs.setLineWrap(true); txtObs.setWrapStyleWord(true);
        ...
        btns.add(btnSalvar); btns.add(btnLimpar); btns.add(btnExcluir); btns.add(btnCancelar);
    ...
    public String getCepSelecionadoId() {
        ...
        if (item.contains(" - ")) return item.split(" - ")[0];
    ...
    public void addTabela(MouseListener l) { tblAlunos.addMouseListener(l); }
    
    public void atualizarTabela(Object[][] d) { 
        tableModel.setRowCount(0); 
        if (d != null) for (Object[] r : d) tableModel.addRow(r); 
    }
    
    public int getLinha() { return tblAlunos.getSelectedRow(); }
    public Object getVal(int r, int c) { return tableModel.getValueAt(r, c); }
    ...
    public LocalDate getDataNasc() { return getLocalDate(spnDataNasc); }
    public String getSexo() { return (String) cmbSexo.getSelectedItem(); }
    public String getEmail() { return txtEmail.getText().trim(); }
    public String getWhatsApp() { return txtWhatsApp.getText().trim(); }
    
    public String getLogradouro() { return txtLogradouro.getText().trim(); }
    public String getNumero() { return txtNumero.getText().trim(); }
    public String getComplemento() { return txtComplemento.getText().trim(); }
    public String getBairro() { return txtBairro.getText().trim(); }
    public String getCidade() { return txtCidade.getText().trim(); }
    public String getEstado() { return (String) cmbEstado.getSelectedItem(); }
    public String getCepObs() { return txtCepObs.getText().trim(); }
    
    public LocalDate getDataMatricula() { return getLocalDate(spnDataMatricula); }
    public String getStatus() { return (String) cmbStatus.getSelectedItem(); }
    public Boolean getIsento() { return chkIsento.isSelected(); }
    public String getMotivoIsencao() { return (String) cmbMotivoIsencao.getSelectedItem(); }
    
    public String getNomeEmergencia() { return txtNomeEmergencia.getText().trim(); }
    public String getTelEmergencia() { return txtTelEmergencia.getText().trim(); }
    public String getParentesco() { return (String) cmbParentesco.getSelectedItem(); }
    
    public Boolean getRestricao() { return chkRestricao.isSelected(); }
    public String getDescricaoRestricao() { return txtRestricao.getText(); }
    public String getMedicamentos() { return txtMedicamentos.getText(); }
    public String getAlergias() { return txtAlergias.getText(); }
    
    public Boolean getImg() { return chkImg.isSelected(); }
    public Boolean getDivulg() { return chkDivulg.isSelected(); }
    public Boolean getAceite() { return chkTermos.isSelected(); }
    public LocalDate getDataAceite() { return getLocalDate(spnDataAceite); }
    
    public LocalDate getDataCancelamento() { return getLocalDate(spnDataCancelamento); }
    public String getMotivoCancelamento() { return txtMotivoCancelamento.getText().trim(); }
    public String getObs() { return txtObs.getText(); }

    // ====== SETTERS ======
    public void setNome(String v) { txtNome.setText(v); }
    public void setCpf(String v) { txtCPF.setText(v); }
    public void setRg(String v) { txtRG.setText(v); }
    public void setDataNasc(LocalDate d) { setLocalDate(spnDataNasc, d); }
    public void setSexo(String v) { cmbSexo.setSelectedItem(v); }
    public void setEmail(String v) { txtEmail.setText(v); }
    public void setWhatsApp(String v) { txtWhatsApp.setText(v); }
    
    public void setLogradouro(String v) { txtLogradouro.setText(v); }
    public void setNumero(String v) { txtNumero.setText(v); }
    public void setComplemento(String v) { txtComplemento.setText(v); }
    public void setBairro(String v) { txtBairro.setText(v); }
    public void setCidade(String v) { txtCidade.setText(v); }
    public void setEstado(String v) { cmbEstado.setSelectedItem(v); }
    public void setCepObs(String v) { txtCepObs.setText(v); }
    
    public void setDataMatricula(LocalDate d) { setLocalDate(spnDataMatricula, d); }
    public void setStatus(String v) { cmbStatus.setSelectedItem(v); }
    public void setIsento(Boolean v) { chkIsento.setSelected(v != null && v); }
    public void setMotivoIsencao(String v) { cmbMotivoIsencao.setSelectedItem(v); }
    
    public void setNomeEmergencia(String v) { txtNomeEmergencia.setText(v); }
    public void setTelEmergencia(String v) { txtTelEmergencia.setText(v); }
    public void setParentesco(String v) { cmbParentesco.setSelectedItem(v); }
    
    public void setRestricao(Boolean v) { chkRestricao.setSelected(v != null && v); txtRestricao.setEnabled(v != null && v); }
    public void setDescricaoRestricao(String v) { txtRestricao.setText(v); }
    public void setMedicamentos(String v) { txtMedicamentos.setText(v); }
    public void setAlergias(String v) { txtAlergias.setText(v); }
    
    public void setImg(Boolean v) { chkImg.setSelected(v != null && v); }
    public void setDivulg(Boolean v) { chkDivulg.setSelected(v != null && v); }
    public void setAceite(Boolean v) { chkTermos.setSelected(v != null && v); spnDataAceite.setEnabled(v != null && v); }
    public void setDataAceite(LocalDate d) { setLocalDate(spnDataAceite, d); }
    public void setDataCancelamento(LocalDate d) { setLocalDate(spnDataCancelamento, d); }
    public void setMotivoCancelamento(String v) { txtMotivoCancelamento.setText(v); }
    public void setObs(String v) { txtObs.setText(v); }

    // ====== LIMPAR FORMULÁRIO ======
    public void limparFormulario() {
        // Dados Pessoais
        txtNome.setText(""); txtCPF.setText(""); txtRG.setText(""); setDataNasc(null); cmbSexo.setSelectedIndex(0);
        txtEmail.setText(""); txtWhatsApp.setText("");
        
        // Endereço
        cmbCep.setSelectedIndex(0);
        txtLogradouro.setText(""); txtNumero.setText(""); txtComplemento.setText("");
        txtBairro.setText(""); txtCidade.setText(""); cmbEstado.setSelectedIndex(0); txtCepObs.setText("");
        
        // Matrícula & Status
        setDataMatricula(LocalDate.now()); cmbStatus.setSelectedIndex(0); 
        chkIsento.setSelected(false); cmbMotivoIsencao.setSelectedIndex(0);
        
        // Emergência
        txtNomeEmergencia.setText(""); txtTelEmergencia.setText(""); cmbParentesco.setSelectedIndex(0);
        
        // Saúde
        chkRestricao.setSelected(false); txtRestricao.setText(""); txtRestricao.setEnabled(false);
        txtMedicamentos.setText(""); txtAlergias.setText("");
        
        // Autorizações & Termos
        chkImg.setSelected(false); chkDivulg.setSelected(false); chkTermos.setSelected(false); 
        spnDataAceite.setEnabled(false); setDataAceite(null);
        
        // Cancelamento
        spnDataCancelamento.setEnabled(false); setDataCancelamento(null); 
        txtMotivoCancelamento.setEnabled(false); txtMotivoCancelamento.setText("");
        
        // Observações & Grid
        txtObs.setText(""); tblAlunos.clearSelection();
